...
def get_hierarchy_relations(hierar_taxonomy, label_map, root=None, fortree=False):
    ...
    with codecs.open(hierar_taxonomy, "r", "utf8") as f:
        for line in f:
            ...
            #print("Parent ",parent_label," ",parent_label_id)
            #print("Children ",children_label," ",children_label_ids)
            if fortree:
                #print(label_tree)
                if (parent_label_id + 1) not in label_tree:
                    print(label_map)
                    print(parent_label_id + 1)
